...
`make iso` produces `chaos.iso`, a bootable CD image. It uses El Torito floppy
emulation: the BIOS boots the embedded 1.44 MiB floppy image as if it were a
real floppy, so the custom bootloader works unchanged in any VM.

```bash
make iso        # builds chaos.iso (needs genisoimage)
make run-iso    # boots the ISO in QEMU, same as a desktop VM would
```

To use it in **VirtualBox** or **VMware**:

1. Create a new VM (type: Other/Unknown, 32-bit, any small amount of RAM).
2. Attach `chaos.iso` to the virtual CD drive and boot from it.
3. (Optional, for the filesystem and `exec`) attach `disk.img` as a raw IDE
   hard disk. VirtualBox needs it wrapped first:

```bash
VBoxManage convertfromraw disk.img chaos-disk.vdi --format VDI
```

Without the hard disk the OS still boots; disk-backed features (`ls`, `cat`,
`exec`) will just report "no disk". Changes written to the converted `.vdi`
do not sync back to `disk.img`.
